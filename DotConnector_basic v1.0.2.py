...
class App:
    def __init__(self, root):
        ...
        self._chk_hotkey_stop = None    # 点击设置页热键复选框
    ...
    def _build_page_click(self):
        ...
        self._chk_hotkey_stop = ModernCheck(
            lf, '热键 %s 开启/停止（默认开启）' % fmt_hotkey(*self.hotkey_cfg['toggle']),
            self.v_hotkey_stop)
        self._chk_hotkey_stop.grid(
    ...
    def _update_hk_entry(self, key):
        ...
        if lbl is not None:
            try:
                ...
                pass
        # 同步更新点击设置页停止条件的热键复选框文本
        if key == 'toggle' and self._chk_hotkey_stop is not None:
            try:
                m, v = self.hotkey_cfg['toggle']
                self._chk_hotkey_stop.set_text(
                    '热键 %s 开启/停止（默认开启）' % fmt_hotkey(m, v))
            except Exception:
                pass
