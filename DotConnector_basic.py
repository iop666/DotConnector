...
def act_desc(typ, flags):
    ...
    if typ == ACT_MOVE:
        return '移动'
    if typ == ACT_DOWN:
        return {0: '左键按下', 1: '中键按下', 2: '右键按下'}.get(flags, '按下')
    if typ == ACT_UP:
        return {0: '左键松开', 1: '中键松开', 2: '右键松开'}.get(flags, '松开')
    if typ == ACT_KEYDOWN:
        return '按键 %s' % vk_to_name(flags)
    if typ == ACT_KEYUP:
        return '松键 %s' % vk_to_name(flags)
    if typ == ACT_WAIT:
        return '等待'
    return '?'
...
class App:
    def __init__(self, root):
        ...
        self._script_total_text = tk.StringVar(value='历史累计: 0 次')
        ...
        if self.v_history_log.get():
            ...
            try:
                ...
                with open(self._log_file, 'a', encoding='utf-8') as f:
                    f.write('== DotConnector v%s 启动于 %s ==\n' % (
    ...
    def _build_page_status(self):
        ...
        ttk.Label(cnt, text='点击次数记录：', style='Top.Sub.TLabel').pack(side='left')
        ttk.Label(cnt, text='本次:', style='Top.Sub.TLabel').pack(side='left', padx=(P(8), 0))
        ...
        ttk.Label(scnt, text='脚本运行记录：', style='Top.Sub.TLabel').pack(side='left')
        ttk.Label(scnt, text='本次:', style='Top.Sub.TLabel').pack(side='left', padx=(P(8), 0))
        ...
        # 三列标题（第0行）
        ttk.Label(hk_card, text='连点模式热键', foreground=CLR_PRIMARY, style='Card.TLabel',
                  font=(FONT_FAMILY, 9, 'bold')).grid(row=0, column=0, sticky='w', padx=P(6), pady=(P(2), P(2)))
        ttk.Label(hk_card, text='脚本模式热键', foreground=CLR_PRIMARY, style='Card.TLabel',
                  font=(FONT_FAMILY, 9, 'bold')).grid(row=0, column=1, sticky='w', padx=P(14), pady=(P(2), P(2)))
        ttk.Label(hk_card, text='强制退出软件热键', foreground=CLR_PRIMARY, style='Card.TLabel',
        ...
        ttk.Button(logbtns, text='清空日志', command=self._clear_log,
                   style='Small.TButton').pack(side='right', padx=P(2))
        ttk.Button(logbtns, text='历史记录', command=self._open_history,
                   style='Small.TButton').pack(side='right', padx=P(2))
        ttk.Button(logbtns, text='导出 TXT', command=lambda: self._export_log('txt'),
                   style='Small.TButton').pack(side='right', padx=P(2))
        ttk.Button(logbtns, text='导出 MD', command=lambda: self._export_log('md'),
                   style='Small.TButton').pack(side='right', padx=P(2))
    ...
    def _build_page_click(self):
        ...
        self._lbl_cap = ttk.Label(lf, text='按 %s 捕获当前坐标' % fmt_hotkey(*self.hotkey_cfg['capture']),
                                  foreground=CLR_SUB)
        ...
        self._lbl_hotkey_stop = ttk.Label(
            lf, text='热键 %s 开启/停止（固定开启）' % fmt_hotkey(*self.hotkey_cfg['toggle']),
        ...
        ModernRadio(lf, '不自动停止', self.v_stop_mode, 'none').grid(
            row=1, column=0, sticky='w', padx=P(6), pady=(P(1) + 7, P(1) + 7))
        ModernRadio(lf, '点击次数:', self.v_stop_mode, 'count').grid(
        ...
        ttk.Label(lf, text='次').grid(row=2, column=2, sticky='w', padx=P(2))
        ModernRadio(lf, '倒计时:', self.v_stop_mode, 'cd').grid(
        ...
        ttk.Label(lf, text='分').grid(row=3, column=2, sticky='w', padx=P(2))
        ttk.Entry(lf, textvariable=self.v_cd_sec, width=5, justify='center').grid(
            row=3, column=3, sticky='w', padx=P(4))
        ttk.Label(lf, text='秒').grid(row=3, column=4, sticky='w', padx=P(2))
        ModernRadio(lf, '运行到:', self.v_stop_mode, 'until').grid(
        ...
        # ⑥ 窗口检测（由原"运行状态"页移入）
        lf = ttk.LabelFrame(inner, text='⑥ 窗口检测', padding=P(6), style='Card.TLabelframe')
        lf.grid(row=row, column=0, sticky='ew', pady=(P(3), 0)); row += 1
        lf.columnconfigure(1, weight=1)
        self._chk_win = ModernCheck(lf, '仅当指定窗口激活时连点，否则自动暂停', self.v_win_on)
        self._chk_win.grid(row=0, column=0, columnspan=2, sticky='w', padx=P(6))
        ttk.Button(lf, text='选择运行中的程序…', command=self._pick_window).grid(
    ...
    def _build_page_log(self):
        ...
        # ===== 脚本录制模块（热键控制，无按钮） =====
        recf = ttk.LabelFrame(page, text='脚本录制', padding=P(6), style='Card.TLabelframe')
        recf.grid(row=0, column=0, sticky='ew', pady=(0, P(4)))
        self._lbl_rec_status = ttk.Label(recf, text='空闲', foreground=CLR_SUB,
                                         style='Card.Sub.TLabel')
        self._lbl_rec_status.grid(row=0, column=0, columnspan=4, sticky='w', padx=P(4), pady=(0, P(2)))
        self._lbl_rec_hk = ttk.Label(recf, text='开始/停止录制：按 %s' % fmt_hotkey(*self.hotkey_cfg['record']),
                                     foreground=CLR_PRIMARY, style='Card.TLabel',
                                     font=(FONT_EN, 9, 'bold'))
        self._lbl_rec_hk.grid(row=1, column=0, sticky='w', padx=P(4), pady=P(1))
        # 键盘录制（第1行）
        self._chk_rec_keys = ModernCheck(recf, '记录键盘输入（常用键+符号键）', self.v_script_keys)
        ...
        ttk.Label(max_row, text='最大步骤:', style='Card.TLabel').pack(side='left')
        ...
        ttk.Label(fmt_row, text='记录格式:', style='Card.TLabel').pack(side='left')
        ModernRadio(fmt_row, 'XML', self.v_script_format, 'xml').pack(side='left', padx=P(4))
        ModernRadio(fmt_row, '二进制', self.v_script_format, 'dcs').pack(side='left')

        # ===== 脚本播放模块（热键控制，无按钮） =====
        playf = ttk.LabelFrame(page, text='脚本播放', padding=P(6), style='Card.TLabelframe')
        ...
        ttk.Label(row0, text='脚本:', style='Card.TLabel').pack(side='left', padx=(P(2), P(2)))
        ...
        ttk.Button(row0, text='刷新', command=self._refresh_script_list,
                   style='Small.TButton').pack(side='left')
        ttk.Button(row0, text='打开脚本文件夹', command=self._open_script_folder,
                   style='Small.TButton').pack(side='left', padx=(P(2), 0))
        # 导入/导出提示（非按钮）
        ttk.Label(playf, text='导入/导出脚本：可放入或移动文件到软件同名目录下的 Scripts 文件夹',
        ...
        ttk.Label(sort_row, text='排序:', style='Card.TLabel').pack(side='left', padx=(P(2), P(2)))
        ModernRadio(sort_row, '名称', self.v_script_sort, 'name').pack(side='left')
        ModernRadio(sort_row, '时间', self.v_script_sort, 'time').pack(side='left', padx=(P(8), 0))
        ttk.Label(sort_row, text='顺序:', style='Card.TLabel').pack(side='left', padx=(P(12), P(2)))
        ModernRadio(sort_row, '正序', self.v_script_order, 'asc').pack(side='left')
        ModernRadio(sort_row, '倒序', self.v_script_order, 'desc').pack(side='left', padx=(P(8), 0))
        # 播放/停止（热键提示）
        self._lbl_play_hk = ttk.Label(playf, text='开始/停止播放：按 %s' % fmt_hotkey(*self.hotkey_cfg['play']),
                                      foreground=CLR_PRIMARY, style='Card.TLabel',
                                      font=(FONT_EN, 9, 'bold'))
        ...
        self._chk_script_once = ModernCheck(once_row, '只执行一轮脚本', self.v_script_once)
        ...
        ttk.Label(self._loop_row, text='循环次数:', style='Card.TLabel').pack(side='left', padx=(P(2), P(2)))
        self._ent_script_loop = ttk.Entry(self._loop_row, textvariable=self.v_script_loop,
                                          width=5, justify='center')
        self._ent_script_loop.pack(side='left')
        self._chk_script_infinite = ModernCheck(self._loop_row, '无限循环', self.v_script_infinite)
        ...
        ttk.Label(self._gap_row, text='循环间隔:', style='Card.TLabel').pack(side='left', padx=(P(2), P(2)))
        ...
        ttk.Label(rate_row, text='速度倍率:', style='Card.TLabel').pack(side='left', padx=(P(2), P(2)))
        ...
        self._chk_script_relative = ModernCheck(rel_row, '相对移动（不跳到录制坐标）', self.v_script_relative)
    ...
    def _build_page_hotkeys(self):
        ...
        row = 0
        # 连点模式热键区
        hkf = ttk.LabelFrame(page, text='连点模式热键', padding=P(6), style='Card.TLabelframe')
        ...
        for i, key in enumerate(('toggle', 'capture')):
            ttk.Label(hkf, text=HK_LABELS[key] + ':', style='Card.TLabel').grid(
        ...
        # 脚本模式热键区
        hkf2 = ttk.LabelFrame(page, text='脚本模式热键', padding=P(6), style='Card.TLabelframe')
        hkf2.grid(row=row, column=0, sticky='ew', pady=(0, P(4))); row += 1
        hkf2.columnconfigure(3, weight=1)
        for i, key in enumerate(('record', 'play')):
            ttk.Label(hkf2, text=HK_LABELS[key] + ':', style='Card.TLabel').grid(
                row=i, column=0, sticky='w', padx=P(4), pady=P(1))
            ent = ttk.Entry(hkf2, width=15, justify='center', state='readonly')
            ent.grid(row=i, column=1, sticky='w', padx=P(2), pady=P(1))
            self._hk_entries[key] = ent
            self._update_hk_entry(key)
            ttk.Button(hkf2, text='设置', width=5,
                       command=lambda k=key: self._capture_hotkey(k)).grid(
                row=i, column=2, sticky='w', padx=P(4), pady=P(1))
        # 强制退出软件（居最下方）
        hkf3 = ttk.LabelFrame(page, text='强制退出软件', padding=P(6), style='Card.TLabelframe')
        hkf3.grid(row=row, column=0, sticky='ew', pady=(0, P(4))); row += 1
        hkf3.columnconfigure(3, weight=1)
        ttk.Label(hkf3, text='强制退出:', style='Card.TLabel').grid(
    ...
    def _build_page_ui(self):
        ...
        # 日志设置模块（历史记录开关）
        lf2 = ttk.LabelFrame(page, text='日志设置', padding=P(6), style='Card.TLabelframe')
        lf2.pack(fill='x', pady=(0, P(4)))
        self._chk_history_log = ModernCheck(
            lf2, '历史记录（记录每次运行日志到 logs/ 会话文件）', self.v_history_log)
        ...
        mf = ttk.LabelFrame(page, text='置顶显示', padding=P(6), style='Card.TLabelframe')
        mf.pack(fill='x', pady=(0, P(4)))
        self._chk_top = ModernCheck(mf, '连点模式置顶计数显示', self.v_top)
        ...
        ttk.Label(top_font_row, text='字体:', style='Card.Sub.TLabel').pack(side='left')
        ttk.Entry(top_font_row, textvariable=self.v_top_font, width=5,
                  justify='center').pack(side='left', padx=P(2))
        ttk.Label(top_font_row, text='透明度:', style='Card.Sub.TLabel').pack(side='left', padx=(P(8), 0))
        ...
        # 脚本模式置顶进度显示
        self._chk_progress_top = ModernCheck(mf, '脚本模式置顶进度显示', self.v_progress_top)
        ...
        ttk.Label(prog_font_row, text='字体:', style='Card.Sub.TLabel').pack(side='left')
        ttk.Entry(prog_font_row, textvariable=self.v_prog_font, width=5,
                  justify='center').pack(side='left', padx=P(2))
        ttk.Label(prog_font_row, text='透明度:', style='Card.Sub.TLabel').pack(side='left', padx=(P(8), 0))
        ...
        # 位置恢复默认按钮
        ttk.Button(mf, text='置顶位置恢复默认', command=self._reset_top_positions,
                   style='Small.TButton').pack(anchor='w', padx=P(4), pady=(P(2), 0))
        # 当前版本模块（显示版本号 + 下载地址链接，置于所有功能最下方）
        cv = ttk.LabelFrame(page, text='当前版本', padding=P(6), style='Card.TLabelframe')
        ...
        url_lbl = ttk.Label(cv, text='软件下载地址：%s' % url, foreground=CLR_PRIMARY,
    ...
    def _open_history(self):
        ...
        try:
            ...
            # 左侧：文件列表
            lf = ttk.LabelFrame(top, text='历史日志文件', padding=P(6),
            ...
            # 右侧：内容预览
            rf = ttk.LabelFrame(top, text='日志内容', padding=P(6),
                                style='Card.TLabelframe')
            ...
            def delete_file():
                ...
                if not sel:
                    return
                fname = listbox.get(sel[0])
                if not messagebox.askyesno('删除历史记录',
                                           '确定删除日志文件 %s 吗？' % fname,
                                           parent=top):
                    return
                try:
                    ...
                    messagebox.showerror('删除失败', str(e), parent=top)
                    return
                listbox.delete(sel[0])
                box.configure(state='normal')
                box.delete('1.0', 'end')
                box.configure(state='disabled')
                self._log('已删除历史日志：%s' % fname)
            ...
            ttk.Button(btnf, text='打开日志文件夹', command=open_folder,
                       style='Small.TButton').pack(side='left', padx=P(2))
            ttk.Button(btnf, text='删除选中', command=delete_file,
                       style='Small.TButton').pack(side='right', padx=P(2))
            ttk.Button(btnf, text='关闭', command=top.destroy,
                       style='Small.TButton').pack(side='right', padx=P(2))
        except Exception as e:
            self._log('打开历史记录失败：%s' % e)
    ...
    def _update_hk_entry(self, key):
        ...
        if key == 'toggle' and self._lbl_hotkey_stop is not None:
            try:
                ...
                self._lbl_hotkey_stop.config(
                    text='热键 %s 开启/停止（固定开启）' % fmt_hotkey(m, v))
            except Exception:
                pass
        ...
        if key == 'capture' and self._lbl_cap is not None:
            try:
                ...
                self._lbl_cap.config(text='按 %s 捕获当前坐标' % fmt_hotkey(m, v))
            except Exception:
                pass
        ...
        if key == 'record' and getattr(self, '_lbl_rec_hk', None) is not None:
            try:
                ...
                self._lbl_rec_hk.config(text='开始/停止录制：按 %s' % fmt_hotkey(m, v))
            except Exception:
                pass
        elif key == 'play' and getattr(self, '_lbl_play_hk', None) is not None:
            try:
                m, v = self.hotkey_cfg['play']
                self._lbl_play_hk.config(text='开始/停止播放：按 %s' % fmt_hotkey(m, v))
            except Exception:
                pass
        ...
        if key in ('toggle', 'force'):
            if getattr(self, '_top_hint', None) is not None:
                try:
                    ...
                    self._top_hint.config(text='连点:%s  急停:%s' % (t, f))
                except Exception:
                    pass
        if key in ('play', 'force'):
            if getattr(self, '_prog_top_hint', None) is not None:
                try:
                    ...
                    f = fmt_hotkey(*self.hotkey_cfg['force'])
                    self._prog_top_hint.config(text='脚本:%s  急停:%s' % (p, f))
                except Exception:
                    pass
    ...
    def _update_script_info(self):
        ...
        try:
            ...
            if sm is None:
                self._lbl_script_info.config(text='脚本：%s（无法读取）' % name)
                return
            count, total_ms, has_keys = sm
            sec = total_ms / 1000.0
            if sec >= 60:
                dur = '%d分%d秒' % (int(sec) // 60, int(sec) % 60)
            else:
                dur = '%.1f秒' % sec
            keys_txt = '已启用' if has_keys else '未启用'
            self._lbl_script_info.config(
                text='脚本说明：录制 %d 步，总时长 %s，键盘录制 %s' % (
                    count, dur, keys_txt))
        except Exception:
            pass
    ...
    def _start_script_record(self):
        ...
        self._log('● 开始录制脚本（%s，上限 %d 步）' % (
            '含键盘' if self.v_script_keys.get() else '仅鼠标', max_actions))
    ...
    def _stop_script_record(self):
        ...
        try:
            ...
            path = filedialog.asksaveasfilename(
                ...
                defaultextension='.%s' % ext,
                filetypes=[('XML 脚本', '*.xml'), ('二进制脚本', '*.dcs')],
                title='保存录制脚本')
            if path:
                ...
                self._log('✔ 脚本已保存：%s' % os.path.basename(path))
                self._refresh_script_list(keep=os.path.basename(path))
        except Exception as e:
            self._log('保存脚本失败：%s' % e)
        finally:
            self.recorder = None
    ...
    def _start_script_play(self):
        if self._playing:
            return
        if self._recording:
            self._log('⚠ 录制中，请先停止录制再播放')
            return
        name = self._script_combo.get()
        if not name:
            self._log('⚠ 请先在脚本列表中选择一个脚本')
            return
        path = os.path.join(self._script_dir, name)
        if not os.path.exists(path):
            self._log('⚠ 脚本文件不存在：%s' % name)
            self._refresh_script_list()
            return
        # 记录执行计数（本次/历史累计）
        ...
        self.player.start()
        self._playing = True
        self._log('▶ 开始播放脚本 %s（倍率%.2fx，循环%s%s）' % (
            name, rate,
            '不循环' if once else ('∞' if self.v_script_infinite.get() else loops),
            '' if gap <= 0 else '，间隔%dms' % gap))
        self._update_script_ui()
    ...
    def _update_script_ui(self):
        ...
        try:
            # 录制
            if self._recording:
                self._lbl_rec_status.config(text='录制中…', foreground=CLR_STOP)
            else:
                self._lbl_rec_status.config(text='空闲', foreground=CLR_SUB)
        except Exception:
            pass
        # 图标随状态切换：录制/播放中→红色
        ...
        try:
            if self._playing and self.player is not None:
                ...
                if self.v_script_once.get():
                    ...
                    loop_disp = '循环轮数：%d/∞' % (self.player.loop_now + 1)
                else:
                    loop_disp = '循环轮数：%d/%d' % (self.player.loop_now + 1, self.player.loops)
                ...
                try:
                    ...
                    self._script_total_text.set('历史累计: %d 次' % self._script_run_total)
                except Exception:
                    pass
                ...
                if getattr(self, '_prog_top', None) is not None:
                    try:
                        ...
                        self._prog_top_hint.config(
                            text='脚本:%s  急停:%s  %s' % (p, f, loop_disp))
                    except Exception:
                        pass
            else:
                try:
    ...
    def _maybe_create_top(self):
        if self._top is not None:
            return
        top, bar, val, hint = self._make_top_window('连点模式置顶计数显示')
        # 关闭按钮绑定
        ...
        hint.config(text='连点:%s  急停:%s' % (toggle_hk, force_hk))
        self._top_hint = hint
    ...
    def _maybe_create_progress_top(self):
        if getattr(self, '_prog_top', None) is not None:
            return
        top, bar, val, hint = self._make_top_window('脚本模式置顶进度显示')
        for c in bar.winfo_children():
            if c.winfo_class() == 'Button':
        ...
        force_hk = fmt_hotkey(*self.hotkey_cfg['force'])
        hint.config(text='脚本:%s  急停:%s  只执行一轮' % (play_hk, force_hk))
