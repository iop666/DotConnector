...
def ensure_icons():
    """返回默认蓝色鼠标图标 HICON（icon.ico，16x16，托盘图标尺寸）。
    图标切换功能已移除，托盘固定使用默认图标。"""
    global _ICON_CACHE
    if _ICON_CACHE:
        return _ICON_CACHE
...
class App:
    def __init__(self, root):
        ...
        self._lbl_hotkey_stop = None    # 点击设置页热键提示（固定开启）
        self._lbl_cap = None            # 点击设置页捕获坐标热键提示
        ...
        # 窗口/任务栏图标：默认蓝色；运行中切换红色（redtubiao），停止恢复默认
        self._icon_paths = {}
        self._cur_hicons = []
    ...
    TAB_ITEMS = ['点击设置', '运行状态', '运行日志', '热键设置', '更多设置']
    ...
    def _build_page_status(self):
        ...
        for i, (key, label) in enumerate(HK_LABELS.items()):
            ...
            lbl.grid(row=i, column=1, sticky='w', padx=P(4), pady=P(2))
    ...
    def _build_page_log(self):
        ...
        ttk.Button(top, text='历史记录', command=self._open_history,
                   style='Small.TButton').pack(side='right', padx=P(2))
